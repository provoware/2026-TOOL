...
class AppContext:
    ...
    def bootstrap(self) -> dict:
        ...
        payload.update({"app_id": APP_ID, "app_name": APP_NAME, "version": APP_VERSION, "self_repair": self._last_repair.as_dict()})
        return payload
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:
        ...
        try:
            ...
            if path == "/api/bootstrap":
                payload = self.app.bootstrap(); payload["previous_unclean"] = self.app.previous_unclean; self._json(HTTPStatus.OK, payload); return
            if path == "/api/manifest":
                manifest = load_json(ROOT / "projekt-manifest.json", {}); status = HTTPStatus.OK if manifest else HTTPStatus.INTERNAL_SERVER_ERROR; self._json(status, manifest or {"error": "Manifest fehlt oder ist ungültig.", "code": "MANIFEST"}); return
            ...
            if path == "/api/todos":
                scope = query.get("scope", ["active"])[0]; self._json(HTTPStatus.OK, {"scope": scope, "items": self._data().list_todos(scope)}); return
            if path == "/api/calendar":
                month = query.get("month", [""])[0]; self._json(HTTPStatus.OK, self._data().calendar_month(month)); return
    ...
    def do_POST(self) -> None:
        ...
        try:
            ...
            if path == "/api/self-repair/run":
                report = self.app.run_self_repair(); status = HTTPStatus.SERVICE_UNAVAILABLE if report.blocking else HTTPStatus.OK; self._json(status, report.as_dict()); return
            if path == "/api/quick-save":
                saved = self.app.store.quick_save(str(data.get("title", "")), str(data.get("text", ""))); self._json(HTTPStatus.OK, {"saved": True, "path": str(saved)}); return
            if path == "/api/todos":
                item = self._data().create_todo(str(data.get("title", "")), description=str(data.get("description", "")), priority=str(data.get("priority", "normal")), due_date=data.get("due_date"), due_time=data.get("due_time")); self._json(HTTPStatus.CREATED, {"todo": item}); return
    ...
    def _pick_project_base(self) -> None:
        ...
        if not picker:
            self._json(HTTPStatus.NOT_IMPLEMENTED, {"error": "KDialog ist nicht verfügbar.", "fallback": str(Path.home()), "code": "PICKER-UNAVAILABLE"}); return
        try:
            ...
            self._json(HTTPStatus.SERVICE_UNAVAILABLE, {"error": str(exc), "code": "PICKER-ERROR"}); return
    ...
    def _handle_api_exception(self, exc: Exception) -> None:
        if isinstance(exc, FileExistsError): self._json(HTTPStatus.CONFLICT, {"error": str(exc), "code": "PROJECT-CONFLICT"})
            ...
            self.app.logger.exception("Datenintegritätsfehler"); self._json(HTTPStatus.SERVICE_UNAVAILABLE, {"error": str(exc), "code": "DATA-INTEGRITY"})
        else:
            self.app.logger.exception("Unbehandelter API-Fehler"); self._json(HTTPStatus.INTERNAL_SERVER_ERROR, {"error": "Interner Fehler. Details wurden protokolliert.", "code": "INTERNAL"})
...
def open_browser(url: str) -> None:
    for candidate in ("chromium", "chromium-browser", "google-chrome", "google-chrome-stable"):
        ...
        if binary:
            subprocess.Popen([binary, url], stdout=subprocess.DEVNULL, stderr=subprocess.DEVNULL); return
...
def main() -> int:
    ...
    app = AppContext(); server = ThreadingHTTPServer((args.host, args.port), Handler); server.app = app; host, port = server.server_address[:2]; url = f"http://{host}:{port}/"; app.logger.info("Start %s %s auf %s", APP_NAME, APP_VERSION, url); print(f"{APP_NAME} {APP_VERSION}: {url}", flush=True)
