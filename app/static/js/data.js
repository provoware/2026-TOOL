...
const DataUI = (() => {
  ...
  function todoRow(todo, archived = false) {
    ...
    action.addEventListener("click", () => archived ? restoreTodo(todo.id, action) : archiveTodo(todo.id, action));
  ...
  async function refreshTodos() {
    ...
    try { const data = await api(`/api/todos?scope=${encodeURIComponent(state.scope)}`); state.todos = data.items || []; root.replaceChildren(); if (!state.todos.length) { empty(root, state.scope === "archive" ? "Das Todo-Archiv ist leer. Mit ↶ lassen sich archivierte Aufgaben später wiederherstellen." : "Noch keine offenen Aufgaben. Oben kann direkt die erste Aufgabe gespeichert werden."); return; } state.todos.forEach((todo) => root.appendChild(todoRow(todo, state.scope === "archive"))); }
    catch (error) { empty(root, `Todo-Daten konnten nicht geladen werden: ${error.message}`); App.log(`Todo-Daten konnten nicht geladen werden: ${error.message}`, "error"); }
  }
  async function refreshSummary() {
    ...
    catch (error) { App.log(`Organisationsstatus konnte nicht aktualisiert werden: ${error.message}`, "warn"); }
  }
  ...
  async function refreshCalendar() {
    ...
    catch (error) { renderCalendarGrid(state.month, {}); App.log(`Kalender konnte nicht aktualisiert werden: ${error.message}`, "warn"); }
  }
  ...
  async function createTodo(event) {
    event.preventDefault(); if (!projectAvailable()) { App.log("Bitte zuerst ein Projekt einrichten. Danach kann die Aufgabe sicher gespeichert werden.", "warn"); $("setup-project").click(); return; }
    const title = $("todo-title").value; if (!title.trim()) { $("todo-state").textContent = "Bitte zuerst einen Aufgabentitel eingeben."; App.log("Aufgabe noch nicht gespeichert: Ein Titel fehlt.", "warn"); $("todo-title").focus(); return; }
    const button = document.querySelector(".todo-add"), task = window.Feedback?.begin("Aufgabe wird gespeichert", "Todo und Kalender werden gemeinsam aktualisiert.");
    button.disabled = true; button.setAttribute("aria-busy", "true"); $("todo-state").textContent = "Speichert …";
    try {
      await api("/api/todos", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ title, due_date: $("todo-date").value || null, due_time: $("todo-time").value || null, priority: $("todo-priority").value }) });
      task?.update(70, "Aufgabe gespeichert; Kalender und Übersicht werden aktualisiert …");
      $("todo-title").value = ""; $("todo-date").value = ""; $("todo-time").value = ""; $("todo-state").textContent = "Gespeichert ✓"; state.scope = "active"; syncArchiveButton(); await refreshAll();
      task?.end("ok", "Aufgabe gespeichert.", "Todo-Liste und Kalender sind aktualisiert."); App.log("Todo gespeichert und Kalender aktualisiert.", "ok", false);
    } catch (error) {
      $("todo-state").textContent = `${error.message} Die Aufgabe wurde nicht als erfolgreich gespeichert markiert.`; task?.end("error", "Aufgabe konnte nicht gespeichert werden.", error.message); App.log(error.message, "error", false);
    } finally { button.disabled = false; button.removeAttribute("aria-busy"); }
  }
  async function archiveTodo(id, button) {
    const task = window.Feedback?.begin("Aufgabe wird archiviert", "Die Aufgabe wird nicht gelöscht und bleibt wiederherstellbar."); button.disabled = true; button.setAttribute("aria-busy", "true");
    try { await api(`/api/todos/${id}/archive`, { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" }); await refreshAll(); task?.end("ok", "Aufgabe archiviert.", "Sie wurde nicht gelöscht und kann wiederhergestellt werden."); App.log("Todo abgehakt und sicher ins Archiv verschoben.", "ok", false); }
    catch (error) { task?.end("error", "Archivieren fehlgeschlagen.", error.message); App.log(error.message, "error", false); }
    finally { button.disabled = false; button.removeAttribute("aria-busy"); }
  }
  async function restoreTodo(id, button) {
    const task = window.Feedback?.begin("Aufgabe wird wiederhergestellt", "Die Aufgabe kehrt in die aktive Liste zurück."); button.disabled = true; button.setAttribute("aria-busy", "true");
    try { await api(`/api/todos/${id}/restore`, { method: "POST", headers: { "Content-Type": "application/json" }, body: "{}" }); await refreshAll(); task?.end("ok", "Aufgabe wiederhergestellt.", "Sie ist wieder in der aktiven Liste."); App.log("Todo aus dem Archiv wiederhergestellt.", "ok", false); }
    catch (error) { task?.end("error", "Wiederherstellen fehlgeschlagen.", error.message); App.log(error.message, "error", false); }
    finally { button.disabled = false; button.removeAttribute("aria-busy"); }
  }
