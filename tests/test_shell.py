...
class ShellTests(unittest.TestCase):
    def test_manifest_and_areas(self):
        ...
        self.assertEqual(manifest["app"]["version"], "0.2.2")
        self.assertEqual(server.APP_VERSION, manifest["app"]["version"])
        self.assertEqual(manifest["iteration"]["number"], 2)
        self.assertEqual(manifest["iteration"]["stage"], "2.3")
        self.assertEqual(manifest["iteration"]["scope"], "release-engineering-only")
        self.assertEqual(manifest["iteration"]["runtime_release"], "0.2.2")
    ...
    def test_html_contains_every_area_and_data_ui(self):
        ...
        for expected in ('id="startup-bar"', 'id="project-dialog"', 'id="todo-form"', 'id="calendar-prev"', '/static/js/data.js', '/static/js/feedback.js'):
    ...
    def test_project_creation_is_isolated_persistent_and_has_database(self):
        with tempfile.TemporaryDirectory() as td:
            config = Path(td) / "config"; base = Path(td) / "projects"; store = server.ProjectStore(config); project = store.create_project(str(base), "Test Projekt")
            self.assertTrue(project["available"]); target = Path(project["path"]); self.assertTrue((target / ".provoware/project.json").is_file())
    ...
    def test_invalid_marker_disables_active_project_without_erasing_configured_path(self):
        with tempfile.TemporaryDirectory() as td:
            config = Path(td) / "config"; store = server.ProjectStore(config); project = store.create_project(str(Path(td) / "projects"), "P"); target = Path(project["path"]); marker = target / ".provoware/project.json"; marker.write_text("{kaputt", encoding="utf-8"); reloaded = server.ProjectStore(config)
            self.assertEqual(reloaded.configured_project_path(), target); self.assertIsNone(reloaded.active_project_path()); self.assertFalse(reloaded.bootstrap()["project"]["available"]); self.assertFalse(reloaded.bootstrap()["project"]["marker_valid"])
    ...
    def test_help_is_machine_readable_and_documents_recovery(self):
        help_data = json.loads((ROOT / "app/static/help.json").read_text(encoding="utf-8")); self.assertGreaterEqual(len(help_data["sections"]), 9); joined = " ".join(section["text"] for section in help_data["sections"])
        for token in ("WAL", "Archiv", "Sicherung", "Self-Repair", "Prozessleiste"): self.assertIn(token, joined)
